...
- Current status: in_progress.
- Current gaps:
  - the new `web/` app now builds, typechecks, runs a small Vitest + Testing Library baseline, routes, proxies `/api`, reads the live Go API through React Query, has auth-aware `/login` plus guarded `/board` routes, and now exercises create/move/archive/restore mutations in the isolated shell
  - the isolated board now keeps mutation ownership scoped to the active card/form/archive row and restores focus after create/move success, but runtime integration with the existing app remains deferred
  - deployment/runtime integration with the existing app remains deferred to later W7/W8 slices
- Corrected gate checklist:
...
- Current status: in_progress.
- Current gaps:
  - the isolated React board now has non-drag create/move/archive/restore plus lane-local move up/down fallback, action-scoped pending ownership, focus continuity for repeated board work, field-level validation, live status feedback, and list-order semantics for assistive tech, but drag-and-drop, keyboard-first reordering polish, and full mobile-first layout work still remain
...
- `W6-P2` Layer split: status `in_progress`. Auth/session plus task/archive/reorder HTTP boundaries, task repository seams, a first task service seam for validation and reorder preconditions, and a dedicated auth persistence/audit service file are now extracted into dedicated files, but broader service/repo separation still remains. Parallel: coordinate with W5 query changes.
...
- `W7-P2` Data layer: status `in_progress`. Typed API reads, an auth-session hook, scoped login/task mutations, a React Query snapshot hook, `401`-aware auth reset behavior, and a frontend unit-test baseline now exist in the isolated shell. Parallel: with W7-P3.
...
- `W7-P4` Page skeletons: status `in_progress`. Overview, login, auth-aware shell navigation, explicit sign-out handling, and guarded board snapshot routes exist for the current isolated-shell scope, including explicit create/move/archive/restore actions and unit-test coverage, but full feature/runtime integration remains incomplete. Parallel: after W7-P1.
### W8
...
- `W8-P4` RWD and a11y: status `in_progress`. The isolated board now has 44px targets, field-level validation, focus recovery, live status feedback, lane-order semantics for assistive tech, and a more explicit signed-in/signed-out shell boundary, but deeper mobile-first layout and keyboard interaction work still remain. Parallel: with W8-P1.
### W9
- `W9-P1` Test gates: status `in_progress`. CI now includes repo-owned Go verification, Windows-local race proof, `web/` scaffold build/typecheck/test, and richer browser smoke for login/create/archive/restore through the shared verify-smoke scripts; this slice begins the browser matrix with `chromium` plus `firefox`, while broader frontend/E2E and deeper matrix work still remain. Parallel: with W9-P2.
- `W9-P2` CI and release flow: status `in_progress`. Workflow hardening now includes the Node 24 JavaScript action runtime pilot, a split verify/smoke job layout, and repo-owned Go/web/smoke verification scripts as the CI source of truth, but release governance remains for later waves. Parallel: partial dependency on W9-P1.
